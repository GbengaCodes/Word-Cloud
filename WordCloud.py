...
def calculate_frequencies(file_contents):
    ...
    word_frequencies={}
    word_list=[]
    #file_collector=open("file_contents", "r")
    for word in file_contents.split():
        """Making each word in the file void of punctuations and uninteresting_words"""
        clean_word=""
        for letters in word.lower():
            if letters not in punctuations:
                if letters.isalpha():
                    clean_word+=letters
        if clean_word not in uninteresting_words:
            word_list.append(clean_word)
    
    for word in word_list:
        """Storing each word and its frequency in dictionary"""
